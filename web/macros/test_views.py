...
class MacrosViewsTestCase(TestCase):
    ...
    def test_macro_list_success(
        self, mock_get_guilds, mock_servers_bot_interface, mock_bot_interface
    ):
        ...
        mock_get_guilds.return_value = [
            {"id": str(self.guild_id), "name": self.guild_name, "permissions": str(0x8)}
        ]
    ...
    @patch("macros.views.get_user_guilds")
    def test_macro_add_post_success(
        self,
        mock_get_guilds,
        ...
        response = self.client.post(
            ...
            {"name": "test_macro", "message": "Test macro message"},
        )
    ...
    def test_macro_add_duplicate_name(self, mock_get_guilds, mock_bot_interface):
        ...
        response = self.client.post(
            reverse("macros:macro_add", args=[self.guild_id]),
            {"name": "test_macro", "message": "New macro message"},
        )

        assert response.status_code == HTTP_OK  # Stay on form with error
    ...
    def test_macro_edit_not_found(
        ...
        response = self.client.get(
            reverse("macros:macro_edit", args=[self.guild_id, "nonexistent"])
        )
    ...
    def test_macro_get_json(self, mock_get_guilds, mock_bot_interface):
        ...
        response = self.client.get(
            reverse("macros:macro_get", args=[self.guild_id, "test_macro"])
        )
        assert response.status_code == HTTP_OK
    ...
    def test_macro_delete_success(self, mock_get_guilds, mock_bot_interface):
        ...
        response = self.client.post(
            reverse("macros:macro_delete", args=[self.guild_id, "test_macro"])
        )
        assert response.status_code == HTTP_REDIRECT  # Redirect after deletion
    ...
    def test_debug_permissions_view(self, mock_get_guilds, mock_bot_interface):
        ...
        response = self.client.get(
            reverse("macros:debug_permissions", args=[self.guild_id])
        )
        assert response.status_code == HTTP_OK
    ...
    def test_macro_add_long_message(self, mock_get_guilds, mock_bot_interface):
        ...
        response = self.client.post(
            reverse("macros:macro_add", args=[self.guild_id]),
            {"name": "test_macro", "message": long_message},
        )
    ...
    def test_macro_add_no_permission(
        ...
        assert response.status_code == HTTP_REDIRECT  # Redirect after permission denied
    ...
    @patch("macros.views._validate_server_access")
    @patch("macros.views.get_user_guilds")
    def test_macro_add_validation_errors_preserved(
        self,
        mock_get_guilds,
        mock_validate_access,
        mock_check_permission,
    ...
    @patch("macros.views._check_server_permission")
    @patch("macros.views._validate_server_access")
    def test_macro_add_server_error_handling(
        self, mock_validate_access, mock_check_permission, mock_bot_interface
    ):
        ...
        response = self.client.post(
            reverse("macros:macro_add", args=[self.guild_id]),
            {"name": "test_macro", "message": "Test message"},
        )

        # Should handle error gracefully and show error message
        assert response.status_code == HTTP_REDIRECT  # Redirect after error
    ...
    def test_validate_server_access_success(self, mock_get_guilds):
        ...
        mock_get_guilds.return_value = [{"id": str(guild_id), "name": "Test Server"}]
    ...
    def test_check_server_permission_with_roles(self, mock_get_guilds, mock_get_roles):
        ...
        mock_get_roles.return_value = [{"name": "Admin", "id": "123"}]
    ...
    @patch("macros.views.get_user_roles_in_guild")
    @patch("macros.views.get_user_guilds")
    def test_check_server_permission_api_error_fallback(
        self, mock_get_guilds, mock_get_roles
    ):
