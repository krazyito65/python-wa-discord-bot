...
class AdminPanelViewsTestCase(TestCase):
    ...
    def test_validate_admin_panel_access_no_access(self):
        ...
        with patch("admin_panel.views.get_user_guilds") as mock_get_guilds:
            ...
            response = self.client.get(
                reverse("admin_panel:dashboard", args=[self.guild_id])
            )
            assert (
                response.status_code == HTTP_REDIRECT
            )  # Redirects to servers:dashboard
    ...
    def test_permission_settings_post_update(self, mock_get_guilds):
        ...
        assert response.status_code == HTTP_REDIRECT  # Redirect after successful update
    ...
    def test_role_settings_get(self, mock_get_guilds):
        ...
        response = self.client.get(
            reverse("admin_panel:role_settings", args=[self.guild_id])
        )
        assert response.status_code == HTTP_OK
    ...
    def test_audit_log_view(self, mock_get_guilds):
        ...
        response = self.client.get(
            reverse("admin_panel:audit_log", args=[self.guild_id])
        )
        assert response.status_code == HTTP_OK
    ...
    def test_reset_to_defaults(self, mock_get_guilds):
        ...
        response = self.client.post(
            reverse("admin_panel:reset_to_defaults", args=[self.guild_id])
        )
        assert response.status_code == HTTP_REDIRECT  # Redirect after reset
    ...
    def test_server_config_creation(self):
        ...
        with patch("admin_panel.views.get_user_guilds") as mock_get_guilds:
            ...
            assert ServerPermissionConfig.objects.filter(
                guild_id=str(self.guild_id)
            ).exists()
