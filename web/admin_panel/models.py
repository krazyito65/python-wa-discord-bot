...
class ServerPermissionConfig(models.Model):
    ...
    def has_permission(self, user_roles: list[str], permission_type: str,
                      guild_permissions: int = 0, is_server_owner: bool = False) -> bool:
        """
        ...
        if self.require_discord_permissions:
            # Administrator permission (0x8) always grants access
            if (guild_permissions & 0x8) == 0x8:
                return True

            # Manage Server permission (0x20) grants admin-level access
            if permission_level in ['admin_only', 'moderators', 'trusted_users'] and (guild_permissions & 0x20) == 0x20:
                return True
    ...
    def _check_moderator_or_admin_roles(self, user_roles_lower: list[str]) -> bool:
        ...
        return any(role in moderator_roles or role in admin_roles for role in user_roles_lower)
...
class ServerPermissionLog(models.Model):
    ...
    changed_by = models.CharField(
        max_length=20,
        help_text="Discord user ID who made the change"
    )

    changed_by_name = models.CharField(
        max_length=100,
        help_text="Discord username who made the change"
    )
