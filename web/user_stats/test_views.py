...
from user_stats.views import (
    _build_base_queryset,
    _determine_activity_level,
    _get_available_channels,
    _get_available_users,
    _get_channel_statistics,
    _get_message_field_for_time_range,
    _get_multi_user_channel_data,
    _get_period_field,
    _get_user_statistics,
    _sort_users_by_activity,
...
class UserStatsViewsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.discord_guild = DiscordGuild.objects.create(
            guild_id=str(self.guild_id), name="Test Server"
        )
    ...
    def test_user_stats_dashboard_success(self, mock_get_guilds):
        ...
        mock_get_guilds.return_value = [
            {"id": str(self.guild_id), "name": "Test Server", "permissions": str(0x8)}
        ]
    ...
    def test_multi_user_channel_stats_success(self, mock_get_guilds):
        ...
        response = self.client.get(
            reverse("user_stats:multi_user_channel", args=[self.guild_id])
        )
        assert response.status_code == HTTP_OK
    ...
    def test_live_stats_update_success(self, mock_get_guilds):
        ...
        response = self.client.get(
            reverse("user_stats:live_stats", args=[self.guild_id])
        )
        assert response.status_code == HTTP_OK
        assert response["Content-Type"] == "application/json"
    ...
    def test_discord_api_error_handling(self, mock_get_guilds):
        ...
        response = self.client.get(reverse("user_stats:dashboard"))
        assert response.status_code == HTTP_OK  # Should handle error gracefully
    ...
    def test_guild_stats_invalid_time_range(self, mock_get_guilds):
        ...
        assert response.status_code == HTTP_OK  # Should handle gracefully
