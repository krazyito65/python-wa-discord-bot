        ...
        for config_path in config_paths:
            if config_path.exists():
                with open(config_path, encoding="utf-8") as f:
                    ...
                    database_url = config.get("django", {}).get(
                        "database_url", "sqlite:///~/weakauras-bot-data/statistics.db"
                    )
...
def check_server_permission(
    member: discord.Member, guild_id: int, permission_type: str
) -> bool:
    ...
    if permission_level == "moderators":
        return _check_moderator_access(member, config) or _check_admin_access(
            member, config
        )
    if permission_level == "trusted_users":
        return (
            _check_trusted_user_access(member, config)
            or _check_moderator_access(member, config)
            or _check_admin_access(member, config)
        )
...
def _check_admin_access(member: discord.Member, config: dict) -> bool:
    ...
    if (
        config.get("require_discord_permissions", True)
...
def _check_moderator_access(member: discord.Member, config: dict) -> bool:
    ...
    # Check Discord permissions first if required
    if config.get("require_discord_permissions", True) and (
        member.guild_permissions.administrator
        or member.guild_permissions.manage_guild
...
def get_permission_error_message(
    permission_type: str, config: dict | None = None
) -> str:
    ...
    """
    if not config:
        return "You need server administrator permissions or the admin role to use this command."
    ...
    if permission_level == "custom_roles":
        return (
            "You need to have one of the configured custom roles to use this command."
        )
